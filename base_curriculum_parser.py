        ...
        try:
            if requests:
                if post_data is not None:
                    resp = requests.post(url, data=post_data, headers=HEADERS, timeout=25)
                else:
...
def parse_specialities(faculty_id):
...
def parse_plans_for_speciality(faculty_id, spec_id):
...
def parse_study_plan_tables(faculty_id, spec_id, plan_id):
    post_data = {
...
def run_curriculum_crawler(faculty_id, faculty_name, output_filename):
    start_time = time.time()
    print(f"=== Starting KFU Curriculum Parser for faculty {faculty_id} ({faculty_name}) ===")
    specialities = parse_specialities(faculty_id)
    ...
    for s_idx, spec in enumerate(specialities, 1):
        ...
        plans = parse_plans_for_speciality(faculty_id, spec_id)
        ...
        for plan in plans:
            ...
            disciplines = parse_study_plan_tables(faculty_id, spec_id, plan_id)
    ...
    print(f"\n=== Successfully saved {len(parsed_plans)} study plans to {output_filename} ({file_size_kb:.1f} KB) in {elapsed:.1f}s ===")
